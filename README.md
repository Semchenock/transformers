...
| **Fine-tuned DistilBERT** | **0.9106** | **0.9117** | **53** |

Улучшение fine-tuning над замороженным baseline: **+0.0476 F1** (+5.52% относительных).
Каждый шаг заметно сокращает число ошибок: 158 → 81 → 53.
...
От чистого клона до работающего демо. На Windows вместо `python`
используйте лаунчер `py` (см. раздел «Установка» ниже).

Модель (~256 МБ) уже лежит в репозитории через **Git LFS**, поэтому обучать
её заново не нужно — при клоне убедитесь, что LFS установлен и веса подтянулись:

```bash
git lfs install
git lfs pull            # догрузит fine_tuned_model/model.safetensors
...
# 2. Запустить демо
pip install gradio
python app.py            # откройте http://127.0.0.1:7860
```

Весь инференс — `app.py`, `api.py`, `compare_utils.py`, `error_analysis.py` —
грузит веса из `fine_tuned_model/`. Если хотите переобучить модель с нуля
(создаст `fine_tuned_model/` заново, ~15 мин на CPU):

```bash
python finetune_utils.py
```

Быстрая проверка, что модель на месте и предсказывает:

```bash
python -c "from predictor import predict; print(predict('This movie was great!'))"
# {'label': 'Positive', 'confidence': 0.98..., 'probabilities': {...}}
```
...
- `fine_tuned_model/` — дообученная модель (~250 МБ, хранится в репозитории через Git LFS)
...
```json
{
  "label": "Positive",
  "confidence": 0.9828,
  "probabilities": {"Negative": 0.0172, "Positive": 0.9828}
}
```
...
print(result["confidence"])  # 0.9828
```
...
- **Модель переуверена.** 14 из 53 ошибок сделаны с уверенностью выше 0.9. Средняя
  уверенность на верных ответах 0.930, на ошибочных — 0.778: разрыв есть, но
  четверть ошибок всё равно «уверенные», поэтому порог по уверенности не отсекает
  их надёжно. Если вероятности нужны как оценка риска, модель требует калибровки
  (temperature scaling).
